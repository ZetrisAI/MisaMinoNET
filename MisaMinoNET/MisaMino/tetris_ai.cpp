...
namespace AI {
	...
	int Evaluate(int& clearScore, double& avg_height, const AI_Param& ai_param, const GameField& last_pool, const GameField& pool, int cur_num,
		...
		// ��߶�
		//int last_min_y[32] = {0};
		int min_y[32] = { 0 }; // maybe lowest point?
		int emptys[32] = { 0 };
		...
		int total_hole = 0;
		int beg_y = -5;
		const int pool_w = pool.width(); // field width (1
		const int pool_h = pool.height(); // field height 
			//last_min_y[31] = -1;
		min_y[31] = -1;
		{
		...
		// ��������
		int x_holes[32] = { 0 }; // 水平方向洞的数量 (Number of holes in the column)
		int y_holes[32] = { 0 }; // 垂直方向洞的数量 (holes in row? )
		int x_op_holes[32] = { 0 }; // 水平方向洞的数量 (Number of accessable holes? )
		...
		*/
		...
		{	// tspin pattern
			// tspin3 code was here, lets write patterns and shit yo

			for (int y = 3; y < pool_h; ++y) { // 3 from top
				...
				for (int x = 1; x < pool_w - 1; ++x) {
					...
					if (((row_y[2] >> (x + 1)) & (7)) != 3			/*110*/				// Input must be  X011 & 0111 = 0011
					   //|| ( (row_y[4] >> (x + 1)) & ( 15 ) ) != 11 /*1101*/

						//|| ((row_y[4] >> (x + 1)) & (15)) != 9		/*1011mask=1001*/	// Input must be  10X1 & 1101 = 1001
						|| ((row_y[4] >> (x + 1)) & (13)) != 9		/*1011mask=1001*/	// Input must be  10X1 & 1101 = 1001
						|| ((row_y[1] >> (x + 1)) & (7)) != 0		/*000*/				// Input must be  X000 & 0111 = 0000
						//|| ( (row_y[0] >> (x + 1)) & ( 3 ) ) != 0 /*00*/
						) {
						continue;
					}
					...
					// smallest y of column 
					// for all rows (y), for all columns (x),
					// if the lowest yval in column x, is not y-1, 
					// OR
					// lowest yval in previous column is y-1?
					// 
					// We want the min height of this col and prev column to be 1 less than our current y
					if (min_y[x] != y - 1 || min_y[x - 1] != y - 1) { // finds center? 
						continue; // skip ???? 
					}
					...
					if (min_y[x + 1] > y) { // 洞判定 (find holes)
						if (x_holes[y - 1] > 0 ||
							x_holes[y + 1] > 1 || // change to 0 later :)
							x_holes[y] > 1 ||
							x_op_holes[y - 1] > 0 ||
							x_op_holes[y + 1] > 0 ||
							x_op_holes[y] > 0)
					...
					}
					{
						...
						e &= (e - 1);
						if (e == 0) { // 底三只剩一空 (3rd row only has 1 space left) (top row)
							//++full;
						}
						else {
							score -= s;
